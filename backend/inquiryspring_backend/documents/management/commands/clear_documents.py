...
from inquiryspring_backend.projects.models import ProjectDocument
import os
import shutil
import glob
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not options['confirm']:
            self.stdout.write(
                self.style.WARNING(
                    '这将删除所有文档数据！请使用 --confirm 参数确认操作'
                )
            )
            return

        self.stdout.write('开始清理文档数据...')
        ...
        self.stdout.write(f'找到 {chunk_count} 个文档分块')
        ...
        try:
            ...
            # 删除文档分块
            deleted_chunks = DocumentChunk.objects.all().delete()
            self.stdout.write(f'已删除 {deleted_chunks[0]} 个文档分块')
